...
saved_name=['SE_NsTTd',num2str(Ndt)];
...
%     tic
    for id=1:len   
        ...
        [SE_DBF(n,id),SE_DBFtx(n,id),DBF]=DBF_water_filling(Hcc,Pb,Ns,noise_power);
         ...
         [SW_Apga_TS(n,id),~]=HBF_APGA_TS(params_pga,params_ts,Pb,Ns,N_rf,Hcc,noise_power,episilon,delta,obj_fun,obj_fun_pg);
         [SW_Rdn_TS(n,id),~]=HBF_TS(params_ts,Pb,Ns,N_rf,Hcc,noise_power,episilon,obj_fun);      

         [SW_rs(n,id),~]=HBF_APGA_TbRs(params_pga,params_TbRs,Pb,Ns,N_rf,Hcc,noise_power,episilon,delta,obj_fun,obj_fun_pg); 
