function [SE_ts,W_rf,TC]=HBF_APGA_TS(params_pga,params_ts,Pb,Ns,N_rf,H_all,noise_power,episilon,delta,obj_fun,obj_fun_pg,Flag,InI,NumNeighbor)
tStart = tic;
[Nr,Nt,Nc]=size(H_all);
Np=10;
power_avr=Pb/Ns;
...
% for c=1:Nc
%     Hc=H_all(:,:,c);
%     H_avr=H_avr+Hc'*InI.proj_comb{c}*Hc;
% end
% H_avr=1/Nc*H_avr;
...
[SE_SW_tx,F_rf,data_pag_ts]=Tabu_search_pga(params_ts,F_rf_ini,Ns,Vec_pgr,Index_bf,Npart1,HBF_tx,coef,obj_fun,NumNeighbor);
...
% [SE_tx,F_rf,iter_tx]=Tabu_search(Len_ts,Niter_ts,Niter_local_ts,F_ini_int,Ns,Srrgt,coef,obj_fun);
...
coef=1/noise_power;
% [~,W_rf,~]=Tabu_search(Len_ts,Niter_ts,Niter_local_ts,W_ini_int,Ns,Srrgt,coef,obj_fun);
...
if length(Index_bf)>=1
[SE_SW_tx,W_rf,iter]=Tabu_search_pga(params_ts,W_rf_ini,Ns,Vec_pgr,Index_bf,Npart1,HBF_info,coef,obj_fun,NumNeighbor);
else
    W_rf=W_pg;
end
