function Neighbor_set=neighbor_generate_MUpga_new(W_rf_ini,Vec_pgr,Index_bf,Npart1,N,Ns,N_rf,Hall,Fbb_all,noise_power,Wgtu)
tol=1e-6;
    ...
    for s=1:neighbor_size
        ...
        if Nsum>=max(N*N_rf/5,N_rf) && Nsum<=(N-1)*N_rf+1% narrow down search space
            ...
            if min(sum(NumActiveAntennas_perRFchain))>=N/5 && rank(Wx_rshp,tol)>=Ns
                ...
                Neighbor_set.SE{num_neighbor}=rate_calcu_MISO(Hall,Wx_rshp,Fbb_all,noise_power,Wgtu);
